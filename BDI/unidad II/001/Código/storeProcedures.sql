...
-- 13.1.16 CREATE PROCEDURES AND CREATE FUNCTIONS 
...
DELIMITER $$
    ...
    CREATE PROCEDURE sp_randomInt(IN NMIN INT, IN NMAX INT, OUT RESULT INT) 
    BEGIN 

        SET RESULT = (SELECT CAST(RAND()*(NMAX-NMIN)+NMIN AS INT));
...
SELECT @result AS "Result of a SubOperation";